...
//                self.setUpCompleteDetailInfo()
                ...
                self.stepper.label.attributedText = self.setSteeperForNoCaseAndOneBottleValue()
...
//        lblMoreInfoText.text = ProductDetailsStorage.shared.productDetailsData.additional_information
    ...
    func caseDiscountText(discountEligible: Bool) -> NSMutableAttributedString {
        var caseDiscountText = ""
        if discountEligible {
            caseDiscountText = caseDiscountText.appending("\n" + "Case Discount!".localizedUppercase)
        }
    ...
    func freeShippingText(free_shipping: Bool) -> NSMutableAttributedString {
        var freeShippingText = ""
        if free_shipping {
            freeShippingText = freeShippingText.appending("\n" + "Free Shipping!".localizedUppercase)
        }
    ...
    func actualPrice(numCase: Int, numBottle: Int, caseActualPrice: Double, bottleActualPrice: Double, currency: String) -> NSMutableAttributedString {
        ...
        var actualPrice = currency + String(finalActualPrice)
    ...
    func discountedPrice(numCase: Int, numBottle: Int, caseDiscountedPrice: Double, bottleDiscountedPrice: Double, currency: String) -> NSMutableAttributedString {
        ...
        var discountedPrice = currency + String(finalDiscountedPrice)
        if finalDiscountedPrice.truncatingRemainder(dividingBy: 1) == 0 {
            discountedPrice = Int(finalDiscountedPrice).description
        }
        
        let attributeStringForDiscountedPrice: NSMutableAttributedString = NSMutableAttributedString(string: String(discountedPrice))
    ...
    func stepperValueChanged(stepper: GMStepper) {
        ...
        if (numCase == 0 && numBottle == 1) {
            stepper.label.attributedText = setSteeperForNoCaseAndOneBottleValue()
        }
        else if (numCase == 0 && numBottle > 1) {
            stepper.label.attributedText = setSteeperForNoCaseAndMoreThanOneBottleValue(numBottle:numBottle)
        }
        else if (numCase == 1 && numBottle == 0) {
            // text for one case without bottle
            stepper.label.attributedText = setSteeperForOneCaseAndNoBottleValue(numCase: numCase)
        }
        else if (numCase == 1 && numBottle == 1) {
            // text for one case and 1 bottle
            stepper.label.attributedText = setSteeperForOneCaseAndOneBottleValue(numCase: numCase, numBottle: numBottle)
        }
        else if (numCase == 1 && numBottle > 1) {
            // text for one case and 1 bottle
            stepper.label.attributedText = setSteeperForOneCaseAndMoreThanBottleValue(numCase: numCase, numBottle: numBottle)
        }
        else if (numCase > 1 && numBottle == 0) {
            // text for more case without bottle
            stepper.label.attributedText = setSteeperForMoreThanOneCaseAndNoBottleValue(numCase: numCase)
        }
        else if (numCase > 1 && numBottle == 1) {
            // text for more case and 1 bottle
            stepper.label.attributedText = setSteeperForMoreThanOneCaseAndOneBottleValue(numCase: numCase, numBottle: numBottle)
        }
        else if (numCase > 1 && numBottle > 1) {
            // text for more cases and more bottles
            stepper.label.attributedText = setSteeperForMoreThanOneCaseAndMoreThanBottleValue(numCase: numCase, numBottle: numBottle)
        }
        else {
            stepper.label.attributedText = setSteeperForNoCaseAndOneBottleValue()
        }
    ...
    func setSteeperForNoCaseAndOneBottleValue() -> NSMutableAttributedString {
        var bottlePrice = ProductDetailsStorage.shared.productDetailsData.bottle_price.description
        if ProductDetailsStorage.shared.productDetailsData.bottle_price.truncatingRemainder(dividingBy: 1) == 0 {
            bottlePrice = Int(ProductDetailsStorage.shared.productDetailsData.bottle_price).description
        }
        
        var discountedPrice = ProductDetailsStorage.shared.productDetailsData.discount_bottle_price.description
        if ProductDetailsStorage.shared.productDetailsData.discount_bottle_price.truncatingRemainder(dividingBy: 1) == 0 {
            discountedPrice = Int(ProductDetailsStorage.shared.productDetailsData.discount_bottle_price).description
        }
        
        let stepperText = "1 " + "bottle".localizedLowercase + "\n" + ProductDetailsStorage.shared.productDetailsData.currency + " " + discountedPrice + "\n"
        
        let attributeString: NSMutableAttributedString = NSMutableAttributedString(string: ProductDetailsStorage.shared.productDetailsData.currency + " " + bottlePrice)
        ...
        let attributeStringStepper: NSMutableAttributedString = NSMutableAttributedString(string: stepperText)
        attributeStringStepper.append(attributeString)
        attributeStringStepper.append(attributeStringCaseDiscount)
        attributeStringStepper.append(attributeStringFreeShipping)
        return attributeStringStepper
    }
    
    func setSteeperForNoCaseAndMoreThanOneBottleValue(numBottle: Int) -> NSMutableAttributedString {
        var bottlePrice = ProductDetailsStorage.shared.productDetailsData.bottle_price.description
        if ProductDetailsStorage.shared.productDetailsData.bottle_price.truncatingRemainder(dividingBy: 1) == 0 {
            bottlePrice = Int(ProductDetailsStorage.shared.productDetailsData.bottle_price).description
        }
        
        let finalActualPrice = Double(bottlePrice)! * Double(numBottle)
...
//        }
        ...
        let combinedPrice = Double(numBottle) * Double(ProductDetailsStorage.shared.productDetailsData.discount_bottle_price.description)!
        
//        let attributeString: NSMutableAttributedString = NSMutableAttributedString(string: ProductDetailsStorage.shared.productDetailsData.currency + " " + String(combinedPrice))

        
        let stepperText = String(numBottle) + " " + "bottles".localizedLowercase + "\n" + ProductDetailsStorage.shared.productDetailsData.currency + " " + String(combinedPrice) + "\n"
        
        let attributeString: NSMutableAttributedString = NSMutableAttributedString(string: ProductDetailsStorage.shared.productDetailsData.currency + " " + String(finalActualPrice))
    ...
    func setSteeperForOneCaseAndNoBottleValue(numCase: Int) -> NSMutableAttributedString {
        var casePrice = ProductDetailsStorage.shared.productDetailsData.case_price.description
        if ProductDetailsStorage.shared.productDetailsData.case_price.truncatingRemainder(dividingBy: 1) == 0 {
            casePrice = Int(ProductDetailsStorage.shared.productDetailsData.case_price).description
        }
        
        let finalActualPrice = casePrice
        let attributeStringActual: NSMutableAttributedString = NSMutableAttributedString(string: ProductDetailsStorage.shared.productDetailsData.currency + " " + String(finalActualPrice))
        
        attributeStringActual.addAttribute(NSStrikethroughStyleAttributeName, value: 1, range: NSMakeRange(0, attributeStringActual.length))
        
        var discountedCasePrice = ProductDetailsStorage.shared.productDetailsData.discount_case_price.description
        if ProductDetailsStorage.shared.productDetailsData.discount_case_price.truncatingRemainder(dividingBy: 1) == 0 {
            discountedCasePrice = Int(ProductDetailsStorage.shared.productDetailsData.discount_case_price).description
        }
        
        
  
        
        let stepperText = "1 " + "case".localizedLowercase + "\n"
        
        let attributeString: NSMutableAttributedString = NSMutableAttributedString(string: ProductDetailsStorage.shared.productDetailsData.currency + " " + discountedCasePrice + "\n")
 ...
 //       attributeString.addAttribute(NSStrikethroughStyleAttributeName, value: 1, range: NSMakeRange(0, attributeString.length))
        ...
        let attributeStringStepper: NSMutableAttributedString = NSMutableAttributedString(string: stepperText)
        attributeStringStepper.append(attributeString)
        attributeStringStepper.append(attributeStringActual)
    ...
    func setSteeperForOneCaseAndOneBottleValue(numCase: Int, numBottle: Int) -> NSMutableAttributedString {
        var casePrice = ProductDetailsStorage.shared.productDetailsData.case_price.description
        if ProductDetailsStorage.shared.productDetailsData.case_price.truncatingRemainder(dividingBy: 1) == 0 {
            casePrice = Int(ProductDetailsStorage.shared.productDetailsData.case_price).description
        }
        ...
        let combinedPrice = Double(discountedCasePrice)! + Double(discountedPrice)!

        
        let stepperText = "1 " + "case".localizedLowercase + " + " + "1 " + "bottle".localizedLowercase  + "\n" + ProductDetailsStorage.shared.productDetailsData.currency + " " + String(combinedPrice) + "\n"
        
        let actualPrice = Double(casePrice)! + Double(bottlePrice)!

        let attributeStringActual: NSMutableAttributedString = NSMutableAttributedString(string: ProductDetailsStorage.shared.productDetailsData.currency + " " + String(actualPrice))
...
//        attributeStringStepper.append(attributeString)
    ...
    func setSteeperForOneCaseAndMoreThanBottleValue(numCase: Int, numBottle: Int) -> NSMutableAttributedString {
        var casePrice = ProductDetailsStorage.shared.productDetailsData.case_price.description
        if ProductDetailsStorage.shared.productDetailsData.case_price.truncatingRemainder(dividingBy: 1) == 0 {
            casePrice = Int(ProductDetailsStorage.shared.productDetailsData.case_price).description
        }
        ...
        let combinedPrice = Double(discountedCasePrice)! + Double(numBottle) * Double(discountedPrice)!
        
        
        let stepperText = "1 " + "case".localizedLowercase + " + " + String(numBottle) + " "  + "bottles".localizedLowercase  + "\n" + ProductDetailsStorage.shared.productDetailsData.currency + " " + String(combinedPrice) + "\n"
        
        let actualPrice = Double(casePrice)! + Double(numBottle) * Double(bottlePrice)!
    ...
    func setSteeperForMoreThanOneCaseAndNoBottleValue(numCase: Int) -> NSMutableAttributedString {
        var casePrice = ProductDetailsStorage.shared.productDetailsData.case_price.description
         if ProductDetailsStorage.shared.productDetailsData.case_price.truncatingRemainder(dividingBy: 1) == 0 {
         casePrice = Int(ProductDetailsStorage.shared.productDetailsData.case_price).description
         }
        ...
        let combinedPrice = Double(numCase) * Double(discountedCasePrice)!

        let stepperText =  String(numCase) + " " + "cases".localizedLowercase +  "\n" + ProductDetailsStorage.shared.productDetailsData.currency + " " + String(combinedPrice) + "\n"
        
        let actualPrice = Double(numCase) * Double(casePrice)!
        
        let attributeString: NSMutableAttributedString = NSMutableAttributedString(string: ProductDetailsStorage.shared.productDetailsData.currency + " " + String(actualPrice))
    ...
    func setSteeperForMoreThanOneCaseAndOneBottleValue(numCase: Int, numBottle:Int) -> NSMutableAttributedString {
        var casePrice = ProductDetailsStorage.shared.productDetailsData.case_price.description
         if ProductDetailsStorage.shared.productDetailsData.case_price.truncatingRemainder(dividingBy: 1) == 0 {
         casePrice = Int(ProductDetailsStorage.shared.productDetailsData.case_price).description
         }
        ...
        let combinedPrice =  Double(numCase) * Double(discountedCasePrice)! + Double(discountedPrice)!

        
        let stepperText = String(numCase) + " " + "cases".localizedLowercase + " + " + "1 " + "bottle".localizedLowercase  + "\n" + ProductDetailsStorage.shared.productDetailsData.currency + " " +  String(combinedPrice) + "\n"
        
        let actualPrice = Double(numCase) * Double(casePrice)! + Double(bottlePrice)!
    ...
    func setSteeperForMoreThanOneCaseAndMoreThanBottleValue(numCase: Int, numBottle:Int) -> NSMutableAttributedString {
        var casePrice = ProductDetailsStorage.shared.productDetailsData.case_price.description
         if ProductDetailsStorage.shared.productDetailsData.case_price.truncatingRemainder(dividingBy: 1) == 0 {
         casePrice = Int(ProductDetailsStorage.shared.productDetailsData.case_price).description
         }
        ...
        let combinedPrice = Double(numCase) * Double(discountedCasePrice)! + Double(numBottle) * Double(discountedPrice)!

        
        let stepperText = String(numCase) + " " + "cases".localizedLowercase + " + " +  String(numBottle) + " " + "bottles".localizedLowercase  + "\n" + ProductDetailsStorage.shared.productDetailsData.currency + " " +  String(combinedPrice) + "\n"
        
        let actualPrice = Double(numCase) * Double(casePrice)! + Double(numBottle) * Double(bottlePrice)!
